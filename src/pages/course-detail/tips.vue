<template>
  <div class="course-detail-tips">
    ...
    <template v-if="lastest.courseType === 2 && !lastest.bought">
      <div class="price">¥ {{ lastest.price }}</div>
      <router-link :to="{ path: '/cart/confirm/' + lastest.id, query: { type: 1 }}">
...
export default {
  ...
  methods: {
    handleClick () {
      ...
      this.$router.push(`/video/${this.lastest.id}?videoId=${this.lastest.videoId}`)
    }
  }
}
</script>
