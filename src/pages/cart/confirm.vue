...
import { getLessonDetail, createOrder, getTrainDetail } from 'api'
export default {
  data () {
    return {
      cartList: []
  ...
  methods: {
    ...
    handleSubmitOrder () {
      const params = {
        type: this.type,
        linkId: this.cartList[0].id
      }
      createOrder(params).then(res => {
        this.$router.push(`/cart/pay/${res.data.orderSn}`)
      })
    },
    // 获取课程详情
    getCourseDetailData () {
      ...
      if(this.type === 2) {
        getTrainDetail(this.$route.params.id).then(res => {
          res.data.imgUrl = res.data.imageUrl
          res.data.price = res.data.amount

          this.cartList = [res.data]
  
          if (this.cartList.length === 0) {
            this.$confirm('暂无数据，是否返回培训套餐中心？', '提示', {
              confirmButtonText: '返回',
              cancelButtonText: '不返回'
            }).then(() => {
